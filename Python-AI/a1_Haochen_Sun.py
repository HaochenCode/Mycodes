...
def treeToString(root):
    ...
    while len(que) > 0:
        temp = que.pop(0)
        level = level - 1
        string = string + str(temp.value)

        if level == 0:
            string = string + '\n'

        level = level + len(temp.subnodes)
        for i in temp.subnodes:
            que.append(i)
...
if __name__ == '__main__':
    ...
    try:
        print(
            f'list(yieldAllValid([255, 16, "foo", 3], toHex)) => {list(yieldAllValid([255, 16, "foo", 3], toHex))}')  # ['ff', '10', '3']
    except NotImplementedError:
