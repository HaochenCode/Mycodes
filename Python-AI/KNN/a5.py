...
def calc_entropy(classifications):
    ls = unique(classifications)
    totalOccur = 0
    for e in ls[1]:
        totalOccur += e
    result = 0
    for i in ls[1]:
        result += -(i/totalOccur) * math.log(i/totalOccur,2)
    return result
...
def calc_information_gain(parent_classifications, classifications_by_val:dict, val_freqs:dict):
    pEntropy = calc_entropy(parent_classifications)
    totalFreq = 0
    wEntropy = 0
    for e in val_freqs.keys():
        totalFreq += val_freqs.get(e)
    for i in classifications_by_val.keys():
        wEntropy += calc_entropy(classifications_by_val.get(i)) * (val_freqs.get(i)/totalFreq)
    return pEntropy - wEntropy
...
class Node:
    ...
    def classify_point(self, point):
        if self.classification is not None:
            ...
            children = self.children.get(val)
            if children is not None:
                return children.classify_point(point)
            return self.children.get(OTHER).classify_point(point)
...
class KNN_Classifier:
    ...
    def calc_euclidean_distance(self, point_a, point_b):
        a = math.pow(point_a[0]-point_b[0],2) + math.pow(point_a[1]-point_b[1],2)
        return math.sqrt(a)
    ...
    def classify_point(self, point, training_points, training_labels):
        ...
        for i in range(len(training_points)):
            heapq.heappush(heap,(self.calc_euclidean_distance(point,training_points[i]),i))
        neighbors = []
        for n in range(self.k):
            index = heapq.heappop(heap)[1]
            neighbors.append(training_labels[index])
        return self.get_top_label(neighbors)